...
<div id="conteneur_membre">
	<div id="banner_member" style="background: url('<?php echo $info_user['fond_fil']; ?>');">
		...
		<div class="title_content_ban whats_new<?php if(!in_array($_SESSION['id_membre'], $amis) && $_GET['id']!=$_SESSION['id_membre']) { echo ' hide'; } ?>">
			<p><?php if($_GET['id']==$_SESSION['id_membre']) { echo 'Quoi de neuf ?'; } elseif(in_array($_SESSION['id_membre'], $amis)) { echo 'Ecrire sur son fil d\'actu'; } else { echo htmlspecialchars($info_user['pseudo']) . ' n\'est pas votre amis, vous ne pouvez pas publier sur son fil.'; } ?></p>
		</div>
		<?php
		if(in_array($_SESSION['id_membre'], $amis) || $_GET['id']==$_SESSION['id_membre'])
		{
		?>
		<div id="write_post">
